...
async function updateListItem(req, res) {
  ...
  if (!updatedListItem) {
    res.status(404).json({message: `No list item was found with the id of ${req.listItem.id}`});
    return;
  }

  const bookDataExpanded = await expandBookData(updatedListItem);

  res.json({ listItem: bookDataExpanded })
}
